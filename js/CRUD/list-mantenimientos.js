function show(list){
    ...
    list.forEach(mantenimiento =>{      
        $("#tblaMantenimientos").append('<tr>' 
            // + '<td>' + mantenimiento.automovil + '</td>'
            + '<td>' + mantenimiento.fechaInicio + '</td>'
            + '<td>' + mantenimiento.fechaFin + '</td>'

            + '<td>' 
                + '<button onclick ="retrieve('+mantenimiento.codigoMantenimiento+')" type = "button" class = "btn btn-primary" data-bs-toggle="modal" data-bs-target="#mdMantenimiento">Consultar</button>'            
            + '</td>' 
        +'</tr>')    
    });
}
